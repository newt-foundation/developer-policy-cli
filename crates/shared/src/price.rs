...
mod tests {
    ...
    fn test_trading_signal_display() {
        ...
        signal.add_price("0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48".to_string(), 1.0); // USDC
        signal.add_price("0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2".to_string(), 2000.0); // WETH
        
        // Add some sample indicators
        signal.add_indicator("market_cap_rank".to_string(), "0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48".to_string(), 3.0);
        signal.add_indicator("market_cap_rank".to_string(), "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2".to_string(), 2.0);
        signal.add_indicator("dma_200".to_string(), "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2".to_string(), 1950.0);
...
impl Display for TradingSignal {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        for (token, price) in &self.prices_usd {
            ...
            prices_obj.insert(format!("{} ({})", token_str, token), serde_json::Value::Number(
                serde_json::Number::from_f64(*price).unwrap_or(serde_json::Number::from(0))
            ));
        }
        json_obj.insert("prices_usd".to_string(), serde_json::Value::Object(prices_obj));
        ...
        for (signal, token_values) in &self.indicators {
            ...
            for (token, value) in token_values {
                let token_str = address_to_coingecko.get(token.as_str()).unwrap();
                signal_obj.insert(format!("{} ({})", token_str, token), serde_json::Value::Number(
                    serde_json::Number::from_f64(*value).unwrap_or(serde_json::Number::from(0))
                ));
            }
            indicators_obj.insert(signal.clone(), serde_json::Value::Object(signal_obj));
        }
        json_obj.insert("indicators".to_string(), serde_json::Value::Object(indicators_obj));
